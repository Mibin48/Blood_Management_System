...
import { mockDonationRecords, mockDonationTrendData, mockHealthChecks } from '../../data/bloodBankMockData';
...
function DonationRow({ d }) {
    ...
    const hc = mockHealthChecks.find(h => h.check_id === d.check_id);
    return (
        <>
            <div onClick={() => setOpen(v => !v)}
                ...
                <div style={{ fontFamily: 'var(--font-sub)', fontWeight: 700, fontSize: 15, color: '#fff' }}>{d.quantity_ml} ml</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text3)' }}>{fmt(d.donation_date)}</div>
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--red)', textAlign: 'left', padding: 0 }}>{d.check_id}</button>
            ...
            {open && hc && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                        <div><div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text3)', marginBottom: 8 }}>HEALTH CHECK</div>
                            {[['Weight', hc.weight + ' kg'], ['Hemoglobin', hc.hemoglobin + ' g/dL'], ['Blood Pressure', hc.blood_pressure], ['Result', hc.eligibility_status]].map(([l, v]) => (
                        ...
                        <div><div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text3)', marginBottom: 8 }}>DONATION DETAILS</div>
                            {[['Donation ID', d.donation_id], ['Bank', d.bank_id], ['Quantity', d.quantity_ml + ' ml'], ['Date', fmt(d.donation_date)]].map(([l, v]) => (
                                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text3)' }}>{l}</span>
                                    <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#fff' }}>{v}</span>
                                </div>
                        ...
                        <div><div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text3)', marginBottom: 8 }}>STOCK IMPACT</div>
                            <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#22c55e' }}>{d.quantity_ml <= 350 ? '+350ml' : '+450ml'} {d.blood_group} added to inventory</div>
                        </div>
                    </div>
                </motion.div>
            )}
...
export default function BloodBankDonations() {
    ...
    const totalVol = mockDonationRecords.reduce((s, d) => s + d.quantity_ml, 0);

    return (
        <BloodBankLayout title="Donations" page="DONATIONS">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[{ label: 'TOTAL DONATIONS', val: String(mockDonationRecords.length), color: '#fff' }, { label: 'TOTAL VOLUME', val: `${totalVol.toLocaleString()} ml`, color: '#fff' }, { label: 'UNIQUE DONORS', val: '4', color: '#fff' }, { label: 'THIS MONTH', val: '14', color: '#fff' }].map(({ label, val, color }, i) => (
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                    ...
                    <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--text3)', marginBottom: 20 }}>Last 6 months</div>
                    <ResponsiveContainer width="100%" height={180}>
                        <ComposedChart data={mockDonationTrendData}>
                            ...
                            <Tooltip content={({ active, payload, label }) => {
                                if (!active || !payload?.length) return null;
                                return <div style={{ background: '#161622', border: '1px solid rgba(217,0,37,0.3)', borderRadius: 8, padding: '10px 14px' }}><div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text3)', marginBottom: 4 }}>{label}</div>{payload.map(p => <div key={p.dataKey} style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: p.color, marginBottom: 2 }}>{p.dataKey}: {p.value}</div>)}</div>;
                            }} cursor={{ fill: 'rgba(255,255,255,0.02)' }} />
                            <Bar yAxisId="ml" dataKey="quantity_ml" name="Volume (ml)" fill="rgba(217,0,37,0.3)" radius={[4, 4, 0, 0]} isAnimationActive animationDuration={700} />
                            <Line yAxisId="ct" type="monotone" dataKey="donations" name="Donations" stroke="#D90025" strokeWidth={2} dot={{ r: 3, fill: '#D90025' }} isAnimationActive animationDuration={700} />
                        </ComposedChart>
                    </ResponsiveContainer>
                </motion.div>
                {/* Table */}
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                    ...
                    {mockDonationRecords.filter(d => d.donor_name.toLowerCase().includes(search.toLowerCase()) || d.donation_id.toLowerCase().includes(search.toLowerCase())).map(d => <DonationRow key={d.donation_id} d={d} />)}
                </motion.div>
            </div>
        </BloodBankLayout>
    );
