...
import { mockBloodStock, mockIncomingRequests, mockDonationRecords, mockDonors, mockBBPayments, mockStockTrendData, mockDonationTrendData } from '../../data/bloodBankMockData';
...
export default function BloodBankDashboard() {
    ...
    const emergencyPending = mockIncomingRequests.filter(r => r.priority === 'Emergency' && r.status === 'Pending');
    const pendingPayments = mockBBPayments.filter(p => p.payment_status === 'Pending');
    const pendingPayTotal = pendingPayments.reduce((s, p) => s + p.amount, 0);
    const pendingReqs = mockIncomingRequests.filter(r => r.status === 'Pending').length;

    const reqCounts = { All: mockIncomingRequests.length };
    mockIncomingRequests.forEach(r => { reqCounts[r.status] = (reqCounts[r.status] || 0) + 1; });
    const filteredReqs = reqTab === 'All' ? mockIncomingRequests : mockIncomingRequests.filter(r => r.status === reqTab);

    const eligibleDonors = mockDonors.filter(d => d.status === 'Eligible').length;
    const coolingDonors = mockDonors.filter(d => d.status === 'Cooling').length;
    const deferredDonors = mockDonors.filter(d => d.status === 'Deferred').length;
    ...
    return (
        <BloodBankLayout title="Dashboard" page="DASHBOARD">
            <AnimatePresence>{showStockModal && <StockUpdateModal onClose={() => setShowStockModal(false)} />}</AnimatePresence>
            <AnimatePresence>{issueRequest && <IssueBloodModal onClose={() => setIssueRequest(null)} request={issueRequest} />}</AnimatePresence>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[
                        ...
                        { icon: Droplets, label: 'DONATIONS THIS MONTH', val: '14', sub: '6,300 ml collected', color: '#fff' },
                        { icon: Inbox, label: 'PENDING REQUESTS', val: String(pendingReqs), sub: '2 emergency · 0 urgent', color: pendingReqs > 0 ? 'var(--red)' : '#fff', pulse: pendingReqs > 0 },
                        { icon: CreditCard, label: 'PENDING PAYMENTS', val: `₹${pendingPayTotal.toLocaleString()}`, sub: `${pendingPayments.length} invoices`, color: '#fff' },
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                        {[`Total: ${totalUnits} units`, `Critical: ${criticalTypes.length} types (${criticalTypes.map(s => s.blood_group).join(', ')})`, `Storage: ${totalUnits}/2000 capacity`, `Last updated: Today 09:00`].map(t => (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={mockStockTrendData}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                        ...
                        <ResponsiveContainer width="100%" height={160}>
                            <BarChart data={mockDonationTrendData} barSize={20}>
                                <CartesianGrid vertical={false} stroke="rgba(255,255,255,0.04)" />
                        ...
                        <div style={{ marginTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 12 }}>
                            <div style={{ fontFamily: 'var(--font-sub)', fontWeight: 700, fontSize: 16, color: '#fff' }}>118 total · 53,100ml</div>
                    ...
                    {filteredReqs.map(req => {
                        ...
                        return (
                            <div key={req.request_id}
                                ...
                                <div><div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#fff' }}>{req.hospital_name}</div></div>
                                ...
                                <div>
                                    ...
                                    {req.status === 'Fulfilled' && <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text3)', cursor: 'pointer' }}>View Issue</span>}
                                </div>
                            </div>
                        );
                    })}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
                        ...
                        {mockDonationRecords.slice(0, 3).map((d, i) => (
                            <div key={d.donation_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 0', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    ...
                                    <div style={{ fontFamily: 'var(--font-sub)', fontWeight: 600, fontSize: 14, color: '#fff' }}>{d.quantity_ml}ml</div>
                                </div>
                            </div>
                        ))}
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
                            {[{ label: 'ELIGIBLE', val: eligibleDonors, color: '#22c55e' }, { label: 'COOLING', val: coolingDonors, color: '#f59e0b' }, { label: 'DEFERRED', val: deferredDonors, color: 'var(--red)' }].map(({ label, val, color }) => (
                        ...
                        {mockDonors.slice(0, 3).map((d, i) => {
                            const stCol = d.status === 'Eligible' ? '#22c55e' : d.status === 'Cooling' ? '#f59e0b' : 'var(--red)';
                            return (
                                <div key={d.donor_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(217,0,37,0.1)', border: '1px solid rgba(217,0,37,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 12, color: 'var(--red)', flexShrink: 0 }}>{initials(d.name)}</div>
                                        <div>
                                            <div style={{ fontFamily: 'var(--font-sub)', fontWeight: 600, fontSize: 13, color: '#fff', display: 'flex', alignItems: 'center', gap: 8 }}>{d.name}<BloodGroupBadge group={d.blood_group} small /></div>
                                            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text3)' }}>{fmt(d.last_donation_date)}</div>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <span style={{ background: 'rgba(0,0,0,0.3)', border: `1px solid ${stCol}40`, borderRadius: 100, padding: '2px 8px', fontFamily: 'var(--font-mono)', fontSize: 9, color: stCol }}>{d.status.toUpperCase()}</span>
                                        {d.status === 'Eligible' && <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--red)', padding: 0 }}>Recall</button>}
                                    </div>
                                </div>
                            );
                        })}
