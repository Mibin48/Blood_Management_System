...
import { mockHealthChecks, mockDonors } from '../../data/bloodBankMockData';
...
function NewHealthCheckModal({ onClose }) {
            ...
            <motion.div initial={{ scale: 0.93, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.93, opacity: 0 }}
                ...
                {done ? (
                    <div style={{ textAlign: 'center', padding: '32px 0' }}>
                        ...
                        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--red)', marginBottom: 6 }}>HC-005 recorded</div>
                    ...
                    <>
                        ...
                        <select value={donorId} onChange={e => setDonorId(e.target.value)} style={{ ...iS, cursor: 'pointer' }}>
                            ...
                            {mockDonors.map(d => <option key={d.donor_id} value={d.donor_id} style={{ background: '#0F0F17' }}>{d.name} · {d.blood_group}</option>)}
                        </select>
                        ...
                        <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
                            ...
                            <button onClick={() => { setLoading(true); setTimeout(() => { setLoading(false); setDone(true); }, 1500); }} disabled={loading || !donorId} style={{ flex: 2, background: 'var(--red)', border: 'none', borderRadius: 10, padding: '12px 0', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, color: '#fff', opacity: (loading || !donorId) ? 0.5 : 1 }}>
...
export default function BloodBankHealthChecks() {
    ...
    const eligibleCount = mockHealthChecks.filter(h => h.eligibility_status === 'Eligible').length;
    const deferredCount = mockHealthChecks.filter(h => h.eligibility_status === 'Deferred').length;
    const filtered = mockHealthChecks.filter(h => {
        const mt = tab === 'All' || h.eligibility_status === tab;
        const mq = h.donor_name.toLowerCase().includes(search.toLowerCase()) || h.check_id.toLowerCase().includes(search.toLowerCase());
        return mt && mq;
    });

    return (
        <BloodBankLayout title="Health Checks" page="HEALTH-CHECKS">
            <AnimatePresence>{showModal && <NewHealthCheckModal onClose={() => setShowModal(false)} />}</AnimatePresence>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[{ label: 'TOTAL CHECKS', val: String(mockHealthChecks.length), color: '#fff' }, { label: 'ELIGIBLE RESULTS', val: String(eligibleCount), color: '#22c55e' }, { label: 'DEFERRED RESULTS', val: String(deferredCount), color: 'var(--red)' }, { label: 'THIS MONTH', val: '2', color: '#fff' }].map(({ label, val, color }, i) => (
                        ...
                        return (
                            <div key={hc.check_id}>
                                <div onClick={() => setOpenRow(isOpen ? null : hc.check_id)}
                                    ...
                                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text3)' }}>{hc.check_id}</div>
