...
import { mockBBPayments, mockPaymentTrendData } from '../../data/bloodBankMockData';
...
export default function BloodBankPayments() {
    ...
    const [payments, setPayments] = useState(mockBBPayments);
    ...
    const filtered = payments.filter(p => {
        ...
        const mq = p.payment_id.toLowerCase().includes(search.toLowerCase()) || p.hospital_name.toLowerCase().includes(search.toLowerCase());
        return mf && mq;
    });

    const markPaid = (id) => setPayments(prev => prev.map(p => p.payment_id === id ? { ...p, payment_status: 'Paid' } : p));
        ...
        <BloodBankLayout title="Payments" page="PAYMENTS">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} style={{ background: '#0F0F17', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: 28 }}>
                    ...
                    <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--text3)', marginBottom: 20 }}>Last 6 months — received vs pending</div>
                    <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={mockPaymentTrendData} barGap={4}>
                            ...
                            <Bar dataKey="received" name="received" fill="#22c55e" radius={[4, 4, 0, 0]} isAnimationActive animationDuration={700} />
                    ...
                    {filtered.map((p, i) => {
                        ...
                        return (
                            <div key={p.payment_id}
                                ...
                                <div><div style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#fff' }}>{p.hospital_name}</div></div>
                                ...
                                <div>
                                    {paid ? <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text3)', padding: 0 }}>Receipt →</button>
                                        : <button onClick={() => markPaid(p.payment_id)} style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 7, padding: '5px 10px', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text2)' }}>Mark Paid</button>}
                                </div>
                            </div>
                        );
                    })}
