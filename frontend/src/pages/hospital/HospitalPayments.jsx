...
import { mockPayments, mockPaymentChartData } from '../../data/hospitalMockData';
...
export default function HospitalPayments() {
    ...
    const totalPaid = mockPayments.filter(p => p.payment_status === 'Paid').reduce((s, p) => s + p.amount, 0);
    const totalPending = mockPayments.filter(p => p.payment_status === 'Pending').reduce((s, p) => s + p.amount, 0);

    const filtered = mockPayments.filter(p => {
        const mf = filter === 'All' || p.payment_status === filter;
        const mq = p.payment_id.toLowerCase().includes(search.toLowerCase()) || p.bank_name.toLowerCase().includes(search.toLowerCase());
        return mf && mq;
    });
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[
                        ...
                        { label: 'TOTAL THIS MONTH', val: '₹8,500', color: '#fff' },
                        { label: 'TRANSACTIONS', val: mockPayments.length, color: '#fff' },
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={mockPaymentChartData} barGap={4}>
                            ...
                            <Bar dataKey="paid" name="paid" fill="#22c55e" radius={[4, 4, 0, 0]} isAnimationActive animationDuration={700} />
                            <Bar dataKey="pending" name="pending" fill="#f59e0b" radius={[4, 4, 0, 0]} isAnimationActive animationDuration={700} />
                        </BarChart>
                    </ResponsiveContainer>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }}
                    ...
                    {/* Table head */}
                    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr 110px 100px 100px 100px 90px', gap: 12, marginBottom: 8, paddingBottom: 8, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                        {['PAYMENT ID', 'BLOOD BANK', 'REQUEST', 'AMOUNT', 'DATE', 'STATUS', 'ACTION'].map(h => <div key={h} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</div>)}
                    </div>
                    {filtered.map((p, i) => (
                        <div key={p.payment_id}
                            style={{ display: 'grid', gridTemplateColumns: '120px 1fr 110px 100px 100px 100px 90px', gap: 12, alignItems: 'center', padding: '14px 0', borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', transition: 'background 0.15s' }}
