...
import { mockAllHealthChecks } from '../../data/adminMockData';
        ...
        <AdminLayout title="Health Checks" page="HEALTH CHECKS">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[{ l: 'TOTAL CHECKS', v: '9,840' }, { l: 'ELIGIBLE', v: '8,920', c: '#22c55e' }, { l: 'DEFERRED', v: '920', c: 'var(--red)' }, { l: 'THIS MONTH', v: '240' }].map(({ l, v, c }, i) => (
                    ...
                    {mockAllHealthChecks.map(hc => {
                        ...
                        return (
                            <div key={hc.check_id} style={{ display: 'grid', gridTemplateColumns: '70px 1fr 1fr 80px 50px 80px 80px 80px', gap: 10, alignItems: 'center', padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}
                                ...
                                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#fff' }}>{hc.check_id}</div>
                                <div><div style={{ fontFamily: 'var(--font-sub)', fontWeight: 600, fontSize: 13, color: '#fff' }}>{hc.donor_name}</div><div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text3)' }}>{hc.donor_id}</div></div>
                                <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text3)' }}>{hc.bank_name}</div>
                                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text3)' }}>{fmt(hc.check_date)}</div>
                                <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#fff' }}>{hc.weight}<span style={{ fontSize: 10, color: 'var(--text3)' }}>kg</span></div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#fff' }}>{hc.hemoglobin}</span>{hc.hemoglobin >= 12.5 ? <CheckCircle size={12} color="#22c55e" /> : <AlertCircle size={12} color="#f59e0b" />}</div>
                                <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#fff' }}>{hc.blood_pressure}</div>
                                <span style={{ background: ok ? 'rgba(34,197,94,0.1)' : 'rgba(217,0,37,0.1)', border: `1px solid ${ok ? 'rgba(34,197,94,0.3)' : 'rgba(217,0,37,0.3)'}`, borderRadius: 100, padding: '2px 10px', fontFamily: 'var(--font-mono)', fontSize: 9, color: ok ? '#22c55e' : 'var(--red)' }}>{hc.eligibility_status.toUpperCase()}</span>
                            </div>
                        );
                    })}
                </motion.div>
