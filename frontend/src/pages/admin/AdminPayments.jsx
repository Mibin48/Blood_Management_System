...
import { mockAllPayments, mockSystemTrendData } from '../../data/adminMockData';
...
const revenueData = mockSystemTrendData.map(d => ({ month: d.month, received: Math.round(d.donations * 50), pending: Math.round(d.requests * 30) }));

export default function AdminPayments() {
    const [payments, setPayments] = useState(mockAllPayments);
        ...
        <AdminLayout title="Payments" page="PAYMENTS">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
                    {[{ l: 'TOTAL REVENUE', v: '₹84.2L', c: '#22c55e' }, { l: 'PENDING', v: `₹${pendingAmt.toLocaleString()}`, c: '#f59e0b' }, { l: 'THIS MONTH', v: `₹${(paidAmt + pendingAmt).toLocaleString()}` }, { l: 'TRANSACTIONS', v: '8,102' }].map(({ l, v, c }, i) => (
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                    ...
                    {payments.map(p => (
                        <div key={p.payment_id} style={{ display: 'grid', gridTemplateColumns: '90px 1fr 1fr 90px 70px 80px 80px 90px', gap: 10, alignItems: 'center', padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}
                            ...
                            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--red)' }}>{p.request_id.replace('REQ-2025-', '')}</div>
